...
function App() {
  ...
  const handleStartEdit = (id: string, text: string) => {
    ...
    setEditingKeywordText(text);
  };

  const handleSaveEdit = async (id: string) => {
    const trimmed = editingKeywordText.trim();
    if (!trimmed) {
      showToast('关键词不能为空', 'error');
      return;
    }
    try {
      const updated = await keywordsApi.update(id, { text: trimmed });
                  ...
                  >
                    <div className="flex items-center justify-between">
                          ...
                          {editingKeywordId === keyword.id ? (
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                className="bg-white/10 border border-white/20 rounded-lg px-2 py-1 text-white text-sm outline-none focus:border-blue-500 w-32"
                                autoFocus
                              />
                              <button
                              ...
                              <button
                                ...
                                className="p-1 rounded text-slate-400 hover:bg-slate-500/10"
                              >
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => handleStartEdit(keyword.id, keyword.text)}
